// ============================================================================
// physics.js - EGOBALL Server-Authoritative Physics
// Features: Practice mode, dynamic stadium, goal announcements, countdown
// ============================================================================
...
const POWER_KICK_STRENGTH = 0.85;
...
const GOAL_PAUSE_MS = 5500;        // 5.5 sekund gol dan keyin pauza
const GOAL_COUNTDOWN_MS = 3000;    // 3-2-1 hisoblash
const PRACTICE_TIME_MS = 999 * 60 * 1000; // Amaliyot rejimi - cheksiz vaqt
...
// Stadion o'lchamlari - o'yinchi soniga qarab
function stadiumFor(teamSize) {
  // 1v1 = kichik, 2v2 = o'rta, 3v3 = katta, 4v4 = eng katta
...
class PlayerBody {
  constructor(id, team, username, skills = null) {
    this.id = id;
    this.team = team;
...
class Match {
  constructor(roomId, teamSize, onGoal, onEnd, isPractice = false) {
    ...
    this.goalScorer = null; // So'ngi gol urgan o'yinchini saqlash
    this.countdownActive = false;
    this.countdownUntil = 0;
    this.countdownValue = 0;
  }

  addPlayer(id, team, username, skills = null) {
    const p = new PlayerBody(id, team, username, skills);
  ...
  scoreGoal(concedingTeam) {
    ...
    // Amaliyot rejimida gol hisoblanmaydi
    if (!this.isPractice) {
      this.score[scoringTeam]++;
    }

    // Gol urgan o'yinchini topish
  ...
  tick(dtMs) {
    ...
    if (this.countdownActive) {
      ...
      return; // Hisoblash vaqtida o'yin to'xtatilgan
    }

    const frozen = this.simTimeMs < this.kickoffFreezeUntil;

    // Amaliyot rejimida vaqt cheksiz
  ...
  serialize() {
    return {
      ...
      players: Array.from(this.players.values()).map(p => ({
        ...
        skills: p.skills, sprinting: p.sprinting, sprintCooldown: Math.max(0, p.sprintCooldownEnd - this.simTimeMs),
        goals: p.goals
      }))
    };
  }
