...
function startMatch({mode,type}){
  ...
  updateHud();
  document.getElementById('matchSpectatorBanner').style.display='none';
...
async function beginMultiMatch(room){
  stopAllMatchIntervals(); // safety net: a duplicate trigger must never stack a second set of sync loops on top
  ...
  document.querySelectorAll('.screen').forEach(s=>s.classList.add('hidden'));
  updateHud();
  const specBanner = document.getElementById('matchSpectatorBanner'); if(specBanner){ specBanner.style.display = mySpectator? 'block':'none'; specBanner.textContent = mySpectator? '👁 '+t('spectator') : ''; }
...
socket.on('physicsState', (st)=>{
  ...
  if(me){ camX += (me.x-camX)*0.2; camY += (me.y-camY)*0.2; window.__selfMissingStreak = 0; }
  else if(!mySpectator){
    window.__selfMissingStreak = (window.__selfMissingStreak||0) + 1;
    if(!window.__lastSelfWarnAt || Date.now()-window.__lastSelfWarnAt > 3000){
    ...
    // a few misses in a row (not just one late packet) - ask the server to re-verify our identity
    if(window.__selfMissingStreak===5 && roomCode){
      socket.emit('rejoinRoom', { code: roomCode, oldId: myId }, (res)=>{
