...
// Sprint settings - short, fast burst/slide
const SPRINT_DURATION_MS = 450;
const SPRINT_COOLDOWN_MS = 2200;
const SPRINT_SPEED_MULT = 1.85;
const SPRINT_ACCEL_MULT = 1.9;
const SPRINT_DAMPING_MULT = 1.012; // slightly less damping while sprinting = slide feel

// Player out-of-bounds allowance: player can push past the sideline by this
// many units before being bounced back (~1-2cm equivalent at this scale)
const PLAYER_OOB_MARGIN = 16;

// Goal post physical collision (the two ends of the goal mouth)
const POST_RADIUS = 6;
const POST_B_COEF = 0.8;

// Kickoff center-circle restriction: the team NOT taking the kickoff may not
// enter the center circle until the ball has been touched, or this timeout passes
const CENTER_RESTRICT_TIMEOUT_MS = 10000;
...
class Match {
  constructor(roomId, teamSize, onGoal, onEnd, isPractice = false) {
    ...
    this.events = []; // transient events this tick/broadcast window: {type:'kick'|'powerKick'|'post'|'goal'}
    this.ballTouchedSinceKickoff = false;
    this.centerRestrictUntil = 0;
  }
  ...
  resetFormation() {
    ...
    this.lastTouchName = null;
    this.ballTouchedSinceKickoff = false;
    this.centerRestrictUntil = this.simTimeMs + CENTER_RESTRICT_TIMEOUT_MS;
  }
  ...
  scoreGoal(concedingTeam) {
    ...
    this.events.push({ type: 'goal', team: scoringTeam });
  }
  ...
  stepPlayers(frozen) {
    this.players.forEach(p => {
      ...
      let damping = p.input.kick ? KICKING_DAMPING : PLAYER_DAMPING;
      if (p.sprinting) damping = Math.min(0.999, damping * SPRINT_DAMPING_MULT);
      if (mag > 0) {
        p.vx += (ax / mag) * accel;
        p.vy += (ay / mag) * accel;
      }
      ...
      const marginX = -hw + pr - PLAYER_OOB_MARGIN, marginX2 = hw - pr + PLAYER_OOB_MARGIN;
      const marginY = -hh + pr - PLAYER_OOB_MARGIN, marginY2 = hh - pr + PLAYER_OOB_MARGIN;
      if (p.x < marginX) { p.x = marginX; p.vx *= -PLAYER_B_COEF; }
      if (p.x > marginX2) { p.x = marginX2; p.vx *= -PLAYER_B_COEF; }
      if (p.y < marginY) { p.y = marginY; p.vy *= -PLAYER_B_COEF; }
      if (p.y > marginY2) { p.y = marginY2; p.vy *= -PLAYER_B_COEF; }

      // Kickoff center-circle restriction: the team NOT taking the kickoff
      // cannot enter the center circle until the ball is touched (or timeout)
      const restrictionLifted = this.ballTouchedSinceKickoff || this.simTimeMs >= this.centerRestrictUntil;
      if (!restrictionLifted && p.team !== this.kickoffTeam) {
        const cdx = p.x, cdy = p.y, cd = len(cdx, cdy);
        const minCd = this.stadium.kickOffRadius + pr;
        if (cd < minCd && cd > 0.0001) {
          const nx = cdx / cd, ny = cdy / cd;
          p.x = nx * minCd; p.y = ny * minCd;
          const rad = p.vx * nx + p.vy * ny;
          if (rad < 0) { p.vx -= nx * rad; p.vy -= ny * rad; }
        }
      }
    });
  }
    ...
    this.players.forEach(p => {
      ...
      if (d < minD && d > 0.0001) {
        ...
        if (p.input.kick || p.input.powerKick) {
          ...
          this.events.push({ type: p.input.powerKick ? 'powerKick' : 'kick', playerId: p.id, x: ball.x, y: ball.y });
        } else {
        ...
        this.ballTouchedSinceKickoff = true;
      }
    });
  }
  ...
  stepBallMotion(frozen) {
    ...
    // Goal post collisions (the four post ends: two per goal)
    const postPositions = [
      { x: -hw, y: -goalHalf }, { x: -hw, y: goalHalf },
      { x: hw, y: -goalHalf }, { x: hw, y: goalHalf }
    ];
    for (const post of postPositions) {
      const pdx = ball.x - post.x, pdy = ball.y - post.y, pd = len(pdx, pdy);
      const minPd = POST_RADIUS + ball.radius;
      if (pd < minPd && pd > 0.0001) {
        const nx = pdx / pd, ny = pdy / pd, overlap = minPd - pd;
        ball.x += nx * overlap; ball.y += ny * overlap;
        const rel = ball.vx * nx + ball.vy * ny;
        if (rel < 0) {
          ball.vx -= nx * rel * (1 + POST_B_COEF);
          ball.vy -= ny * rel * (1 + POST_B_COEF);
        }
        this.events.push({ type: 'post', x: ball.x, y: ball.y });
      }
    }
  ...
  serialize() {
    ...
    this.events = [];
    const restrictionActive = !this.ballTouchedSinceKickoff && this.simTimeMs < this.centerRestrictUntil;
    return {
      ...
      centerRestrictActive: restrictionActive,
      isPractice: this.isPractice,
