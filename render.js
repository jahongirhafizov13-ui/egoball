...
function render(){
  ...
  // Jersey-look fill: a soft mottled fabric texture in the team color, instead
  // of a flat disc or a hard shiny "shell" - reads as a kit/jersey, not armor.
  ...
  const JERSEY_TEXTURE_CACHE = window.__jerseyTexCache || (window.__jerseyTexCache = {});
  function getJerseyTexture(color){
    if(!color || !/^#[0-9a-f]{6}$/i.test(color)) color = '#888888';
    if(JERSEY_TEXTURE_CACHE[color]) return JERSEY_TEXTURE_CACHE[color];
    const size = 64;
    const off = document.createElement('canvas'); off.width=size; off.height=size;
    const octx = off.getContext('2d');
    octx.fillStyle = color; octx.fillRect(0,0,size,size);
    // seeded-looking but simple: a handful of soft mottled blotches, darker and
    // lighter, so it reads as woven fabric rather than a flat/shiny fill
    for(let i=0;i<16;i++){
      const bx = Math.random()*size, by = Math.random()*size, br = 5+Math.random()*11;
      octx.beginPath(); octx.arc(bx,by,br,0,Math.PI*2);
      octx.fillStyle = (i%2===0) ? shadeHex(color,-28) : shadeHex(color,22);
      octx.globalAlpha = 0.16 + Math.random()*0.18;
      octx.fill();
    }
    octx.globalAlpha = 1;
    JERSEY_TEXTURE_CACHE[color] = off;
    return off;
  }
  players.forEach(p=>{
    ...
    if(ch && chImg && chImg.complete && chImg.naturalWidth>0){
      ...
      const tex = getJerseyTexture(p.color);
      ctx.save(); ctx.clip();
