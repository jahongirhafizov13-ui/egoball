...
class RoomManager {
  ...
  joinRoom(code, username, socketId, password = null) {
    ...
    if (room.started && room.match && room.match.running) {
      room.spectators.set(socketId, username);
  ...
  setMemberTeam(hostSocketId, targetSocketId, newTeam) {
    if (!['red', 'blue', 'spectator'].includes(newTeam)) return { ok: false, error: 'bad_team' };
    const room = this.roomOf(hostSocketId);
    if (!room) return { ok: false, error: 'no_room' };
    if (room.hostId !== hostSocketId) return { ok: false, error: 'not_host' };

    const isMember = room.members.has(targetSocketId);
    const isSpectator = room.spectators.has(targetSocketId);
    if (!isMember && !isSpectator) return { ok: false, error: 'not_in_room' };

    if (newTeam === 'spectator') {
      const username = isMember ? room.members.get(targetSocketId).username : room.spectators.get(targetSocketId);
      if (isMember) {
        room.members.delete(targetSocketId);
        if (room.match) room.match.removePlayer(targetSocketId);
      }
      room.spectators.set(targetSocketId, username);
    } else {
      if (room.members.size >= room.teamSize * 2 && isSpectator) {
        return { ok: false, error: 'team_full' };
      }
      const username = isMember ? room.members.get(targetSocketId).username : room.spectators.get(targetSocketId);
      if (isSpectator) room.spectators.delete(targetSocketId);
      room.members.set(targetSocketId, { username, team: newTeam });
      if (room.match) {
        room.match.removePlayer(targetSocketId);
        room.match.addPlayer(targetSocketId, newTeam, username);
      }
    }
  ...
  addChatMessage(socketId, message) {
    for (const room of this.rooms.values()) {
      if (room.members.has(socketId) || room.spectators.has(socketId)) {
        ...
        const name = member ? member.username : (room.spectators.get(socketId) || 'Spectator');
