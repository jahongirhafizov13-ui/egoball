// ============================================================================
// rooms.js - room/lobby management with player skills
// ============================================================================
...
class RoomManager {
  ...
  createRoom(hostUsername, hostSocketId, teamSize) {
    ...
    const room = {
      ...
      match: null,
      started: false,
      playerSkills: new Map() // socketId -> skills
    };
  ...
  joinRoom(code, username, socketId, skills = null) {
    ...
    if (room.match) room.match.addPlayer(socketId, team, skills);
  ...
  setPlayerSkills(socketId, skills) {
    for (const room of this.rooms.values()) {
      if (room.members.has(socketId)) {
        room.playerSkills.set(socketId, skills);
        return { ok: true };
      }
    }
  ...
  startMatch(code) {
    ...
    room.members.forEach((m, socketId) => {
      ...
      room.match.addPlayer(socketId, m.team, skills);
    });
    room.match.start();
...
module.exports = { RoomManager, MAX_TEAM_SIZE, DEFAULT_SKILLS, MAX_SKILL_POINTS };
