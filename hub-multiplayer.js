...
function fieldDimsFor(mode){
  const dims = MODES[FIELD_SIZE_FOR_MODE[mode] || mode];
  if(dims) return dims;
  console.warn('[EgoBall] fieldDimsFor got an invalid mode, falling back to 2v2:', mode);
  return MODES['2v2'];
}
...
function joinLocalServer(index){
  ...
  myId = socket.id; // set BEFORE emitting - a same-tick 'matchStarting' broadcast can otherwise arrive before this function's own ack does
...
function joinRoomByCode(code, password){
  ...
  myId = socket.id; // set BEFORE emitting - see joinLocalServer for why
