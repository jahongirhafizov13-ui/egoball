...
const LOCAL_START_GRACE_MS = 6000; // once 2+ players are present, wait this long for more before locking the format
...
function ensureLocalServer(i) {
  ...
  if (!rooms[code]) {
    rooms[code] = {
      ...
      hostId: null, players: [], passwordHash: null, chat: [], startTimer: null
    };
  }
  return rooms[code];
}
...
function finalizeLocalStart(room) {
  clearTimeout(room.startTimer); room.startTimer = null;
  if (room.started) return;
  const total = activeCount(room);
  if (total < 2) return; // still alone - keep waiting for a second player, no timer to reschedule here
  const teamSize = Math.min(MAX_TEAM_SIZE, Math.max(1, Math.ceil(total / 2)));
  room.mode = teamSize + 'v' + teamSize;
  room.cap = teamSize * 2;
  let idx = 0;
  room.players.forEach(p => {
    if (p.spectator) return;
    if (idx < room.cap) { p.team = (idx % 2 === 0) ? 'A' : 'B'; idx++; }
    else { p.spectator = true; p.team = null; } // more people showed up than the locked format fits
  });
  room.started = true;
...
io.on('connection', (socket) => {
  ...
  socket.on('joinLocalServer', ({ index, name }, cb) => {
    ...
    if (room.started) {
      // a match is already running here - late arrivals just spectate
      room.players.push({ id: socket.id, name: name || 'Guest', team: null, spectator: true });
      socketRoom[socket.id] = room.code;
      socket.join(room.code);
      pushSystemMsg(room, `${name || 'Guest'} qo'shildi (tomoshabin)`);
      broadcastRoom(room.code);
      return cb({ ok: true, code: room.code, room: publicRoomState(room), myId: socket.id, isHost: false, spectator: true });
    }

    const isHost = !room.hostId;
    if (isHost) room.hostId = socket.id;
    room.players.push({ id: socket.id, name: name || 'Guest', team: null, spectator: false });
    socketRoom[socket.id] = room.code;
    socket.join(room.code);
    pushSystemMsg(room, `${name || 'Guest'} qo'shildi`);

    const total = activeCount(room);
    if (total >= MAX_TEAM_SIZE * 2) {
      finalizeLocalStart(room); // hit the hard cap (5v5) - start right away, no need to wait
    } else if (total >= 2) {
      // enough for at least 1v1 - give a short window for more players to join before locking the format
      clearTimeout(room.startTimer);
      room.startTimer = setTimeout(() => finalizeLocalStart(room), LOCAL_START_GRACE_MS);
      broadcastRoom(room.code);
    } else {
      broadcastRoom(room.code); // first player here - just wait for someone else
    }

    cb({ ok: true, code: room.code, room: publicRoomState(room), myId: socket.id, isHost, spectator: false });
  });
  ...
  socket.on('listPrivateRooms', (cb) => {
    const list = Object.values(rooms)
      .filter(r => !r.isPublic && !r.isLocal && !r.started && !r.passwordHash)
      .map(r => ({
        code: r.code, hostName: (r.players.find(p => p.id === r.hostId) || {}).name || '?',
        count: activeCount(r), cap: r.cap, hasPassword: false
      }));
    cb(list);
  });
