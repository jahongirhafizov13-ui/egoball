...
function ensureLocalServer(i) {
  ...
  if (!rooms[code]) {
    rooms[code] = {
      code, mode: '5v5', cap: MAX_TEAM_SIZE * 2, isPublic: true, isLocal: true, started: true,
      hostId: null, players: [], passwordHash: null, chat: []
    };
  }
  return rooms[code];
...
io.on('connection', (socket) => {
  ...
  socket.on('quickPlay', ({ name, mode }, cb) => {
  ...
  socket.on('joinLocalServer', ({ index, name }, cb) => {
    ...
    socket.join(room.code);
    pushSystemMsg(room, `${name || 'Guest'} qo'shildi`);
    // let existing members' live match know the roster changed (client reconciles without a reset)
    socket.to(room.code).emit('roomUpdate', publicRoomState(room));
    cb({ ok: true, code: room.code, room: publicRoomState(room), myId: socket.id, isHost, spectator });
  });
