...
const CASE_ITEMS = [
  { id:'isagi',   rank:'Common',    chance:24,  speed:1.50, kickPower:3.30, power:1.35, control:0.50 },
  { id:'bachira', rank:'Common',    chance:25,  speed:1.75, kickPower:3.00, power:1.10, control:0.70 },
  { id:'aiku',    rank:'Rare',      chance:20,  speed:1.65, kickPower:3.50, power:1.80, control:0.70 },
  { id:'rin',     rank:'Epic',      chance:15,  speed:1.85, kickPower:4.90, power:1.60, control:0.75 },
  { id:'lorenzo', rank:'Legendary', chance:10,  speed:1.90, kickPower:4.00, power:2.10, control:0.85 },
  { id:'sae',     rank:'Legendary', chance:5,   speed:1.95, kickPower:5.20, power:1.50, control:0.98 },
  { id:'kaiser',  rank:'Legendary', chance:1,   speed:2.10, kickPower:6.00, power:1.75, control:0.80 }
];
...
function publicRoomState(room) {
  return {
    ...
    phase: room.phase || null, kickoffAt: room.kickoffAt || 0, waitingCount: (room.waitingQueue||[]).length
  };
}
...
// Local servers auto-size the pitch to however many real players are actually there right now -
// this is the whole point of "walk-in" local servers (vs. quickPlay where you pick a size upfront).
function computeLocalMode(count){
  if(count<=1) return null;   // 0 or 1 real player = practice/idle, no real match yet
...
function ensureLocalServer(i) {
  ...
  if (!rooms[code]) {
    rooms[code] = {
      code, mode: null, cap: MAX_TEAM_SIZE * 2, isPublic: true, isLocal: true, started: false,
      // phase: 'idle' (0-1 real players, practice ball, no timer/goals) -> 'countdown' (5s, positions set)
      // -> 'playing' (real match, current mode locked in) -> back to 'countdown' after each goal/reset
      phase: 'idle',
      kickoffAt: 0,          // ms timestamp - countdown end / kickoff-restriction grace period reference
      waitingQueue: [],      // ids of players who joined mid-match; folded in at the next kickoff (winner-stays)
      hostId: null, players: [], passwordHash: null, chat: []
    };
  }
  return rooms[code];
...
function removePlayerFromRoom(socket) {
  ...
  if (room.players.length === 0) {
    if (room.isLocal) {
      room.hostId = null;
      room.started = false;
      room.phase = 'idle';   // back to practice mode until 2+ real players return
      room.mode = null;
      room.waitingQueue = [];
      room.kickoffAt = 0;
    } else {
  ...
  if (room.isLocal && activeCount(room) <= 1 && room.phase !== 'idle') {
    // down to a lone player (or nobody active) mid-match - drop back to practice immediately
    room.phase = 'idle'; room.mode = null; room.started = false;
    room.waitingQueue = []; room.kickoffAt = 0;
    room.players.forEach(p => { p.waiting = false; });
    io.to(code).emit('roomUpdate', publicRoomState(room));
    io.to(code).emit('localServerIdle'); // tells the (possibly lone) remaining client to switch to practice mode
  }
...
io.on('connection', (socket) => {
  ...
  socket.on('quickPlay', ({ name, mode, color, characterId, auraId }, cb) => {
    if (!MODE_TEAM_SIZE[mode]) return cb({ ok: false, error: 'bad_mode' });
    ...
    room.players.push({ id: socket.id, name: name || 'Guest', team, spectator, color: color || null, characterId: characterId || null, auraId: auraId || null });
  ...
  socket.on('listLocalServers', (cb) => {
    ...
    for (let i = 1; i <= LOCAL_SERVER_COUNT; i++) {
      ...
      list.push({ index: i, count: activeCount(room), cap: MAX_TEAM_SIZE * 2, mode: room.mode, phase: room.phase });
    }
    cb(list);
  });

  socket.on('joinLocalServer', ({ index, name, color, characterId, auraId }, cb) => {
    const room = ensureLocalServer(index);
    const isHost = !room.hostId;
    if (isHost) room.hostId = socket.id;

    const full = activeCount(room) >= room.cap; // only overflow beyond 5v5 (10) becomes a true spectator
    const midMatch = room.phase === 'playing' || room.phase === 'countdown';
    // Joining while a match is already under way doesn't resize the pitch under everyone's feet -
    // you're folded in at the next kickoff (goal reset / match end) instead, per the winner-stays flow.
    const waiting = !full && midMatch;
    const spectator = full;
    const team = (spectator || waiting) ? null : pickTeam(room);
    const player = { id: socket.id, name: name || 'Guest', team, spectator, waiting, color: color || null, characterId: characterId || null, auraId: auraId || null };
    room.players.push(player);
    if (waiting) room.waitingQueue.push(socket.id);
    socketRoom[socket.id] = room.code;
    socket.join(room.code);
    pushSystemMsg(room, `${name || 'Guest'} qo'shildi`);
    io.to(room.code).emit('playerJoinedNotice', { name: name || 'Guest' });

    if (room.phase === 'idle') {
      const newMode = computeLocalMode(activeCount(room));
      if (newMode) {
        // 0/1 -> 2+ real players: leave practice mode, lock in the mode for this player count,
        // and start the 5-second kickoff countdown.
        room.mode = newMode;
        room.phase = 'countdown';
        room.kickoffAt = Date.now() + 5000;
        room.started = true;
        io.to(room.code).emit('matchStarting', publicRoomState(room));
      } else {
        // still alone - stay in practice/idle, just refresh everyone's lobby view
    ...
    cb({ ok: true, code: room.code, room: publicRoomState(room), myId: socket.id, isHost, spectator, waiting });
  });

  // ---- private rooms (optional password) ---------------------------------
  socket.on('createRoom', async ({ mode, name, cap, password, color, characterId, auraId }, cb) => {
    ...
    const room = {
      ...
      hostId: socket.id, players: [{ id: socket.id, name: name || 'Host', team: 'A', spectator: false, color: color || null, characterId: characterId || null, auraId: auraId || null }],
      passwordHash, chat: []
    };
    rooms[code] = room;
  ...
  socket.on('joinRoomByCode', async ({ code, name, cap, password, color, characterId, auraId }, cb) => {
    const room = rooms[code];
