...
// Features: Google OAuth (JWT verification), E-Coin economy, bot matches, skins, team color
// ============================================================================
...
io.on('connection', (socket) => {
  ...
  // ========== GOOGLE AUTH (JWT credential from Google Sign-In) ==========
  socket.on('googleAuth', async ({ credential } = {}, cb) => {
    cb = cb || (() => {});
    if (rateLimited(socket, 'googleAuth', 2000)) return cb({ ok: false, error: 'too_fast' });
    if (!credential) return cb({ ok: false, error: 'missing_credential' });

    const result = await auth.googleAuth(credential);
    if (result.ok) socket.data.user = result.profile.username;
    cb(result);
  });

  socket.on('setPlayerInfo', async ({ displayName, playerNumber, teamColor } = {}, cb) => {
    cb = cb || (() => {});
    if (!socket.data.user) return cb({ ok: false, error: 'not_authenticated' });
    const result = await auth.setPlayerInfo(socket.data.user, displayName, playerNumber, teamColor);
    cb(result);
  });
  ...
  socket.on('joinLocalServer', ({ serverId } = {}, cb) => {
    ...
    const result = rooms.joinLocalServer(serverId, displayName, socket.id, user);
  ...
  socket.on('createRoom', ({ teamSize, isPrivate, password } = {}, cb) => {
    ...
    const hostUser = auth.getUserByUsername(socket.data.user);
    const hostDisplayName = hostUser ? (hostUser.displayName || hostUser.username) : socket.data.user;
    const room = rooms.createRoom(hostDisplayName, socket.id, size, isPrivate, password);
  ...
  socket.on('joinRoom', ({ code, password } = {}, cb) => {
    ...
    const joinUser = auth.getUserByUsername(socket.data.user);
    const joinDisplayName = joinUser ? (joinUser.displayName || joinUser.username) : socket.data.user;
    const result = rooms.joinRoom((code || '').toUpperCase(), joinDisplayName, socket.id, password);
    if (!result.ok) return cb(result);
    socket.join(result.room.code);
    cb({ ok: true, code: result.room.code, team: result.team, teamSize: result.room.teamSize });
    broadcastRoomUpdate(result.room.code);
  ...
  socket.on('setMemberTeam', ({ targetId, team } = {}, cb) => {
    cb = cb || (() => {});
    if (!requireAuth(cb)) return;
    const result = rooms.setMemberTeam(socket.id, targetId, team);
    if (!result.ok) return cb(result);
    cb({ ok: true });
    broadcastRoomUpdate(result.room.code);
  });
  ...
  function broadcastRoomUpdate(code) {
    ...
    if (!room) return;
    const members = Array.from(room.members.entries()).map(([id, m]) => ({ id, username: m.username, team: m.team }));
    const spectators = Array.from(room.spectators.entries()).map(([id, username]) => ({ id, username }));
