...
io.on('connection', (socket) => {
    ...
    try {
      ...
      cb({
        ok: true, profile: {
          ...
          level: account.level || 1, cups: account.cups || 0,
  ...
  socket.on('hubChat', async ({ msg }) => {
    ...
    io.to('hub').emit('hubChatUpdate', { name, msg, level, cups });
  });
  ...
  socket.on('joinLocalServer', ({ index, name, color, characterId, auraId }, cb) => {
    ...
    const player = { id: socket.id, name: name || 'Guest', team: null, spectator: willQueue, color: color || null, characterId: characterId || null, auraId: auraId || null };
  ...
  socket.on('createRoom', async ({ mode, name, cap, password, color, characterId, auraId, roomName }, cb) => {
    if (socketRoom[socket.id]) removePlayerFromRoom(socket);
  ...
  socket.on('chatMessage', async ({ code, name, msg }) => {
    ...
    room.chat.push({ name, msg, level, cups });
    if (room.chat.length > 40) room.chat.shift();
