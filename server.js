...
const io = new Server(server, {
  // Set ALLOWED_ORIGIN in your VPS environment (e.g. in the systemd service file or .env)
  // to your real deployed site's exact URL once you know it - until then this stays open.
  cors: { origin: process.env.ALLOWED_ORIGIN || '*' },
  // Default heartbeat (25s interval / 20s timeout) means a dead connection can
  // take up to ~45s to even be noticed - far longer than the reconnect grace
  // window below can cover. A tighter heartbeat surfaces real drops in a few
  // seconds so the grace period actually has time to do its job.
  pingInterval: 10000,
  pingTimeout: 8000
});
...
// ---------------------------------------------------------------------------
// Reconnect grace period. socket.io hands out a brand new socket.id on every
// reconnect, so without this, ANY blip (wifi hiccup, phone switching from
// wifi to mobile data, a few seconds of packet loss) meant the player was
// instantly deleted from the match on the server, while their old, now-dead
// player entry just sat frozen on the pitch for everyone else - "u menga
// ko'rinmayapti / muzlab qoldi" was this, not a physics bug.
// nameLower -> { code, oldSocketId, timeoutHandle }
// ---------------------------------------------------------------------------
...
// Called on disconnect instead of removePlayerFromRoom directly. If the
// socket was an active (non-spectator) player in a match that has already
// started, we hold their seat open for RECONNECT_GRACE_MS instead of
// deleting them immediately - see pendingRejoin above. Anything else (menu,
// lobby, spectator, or a room that hasn't started) is removed right away,
// exactly like before.
function scheduleRemovalOrKeep(socket) {
  const code = socketRoom[socket.id];
  const room = code ? rooms[code] : null;
  const nameLower = socket.data && socket.data.accountName;
  if (room && room.started && nameLower) {
    const leaving = room.players.find(p => p.id === socket.id);
    if (leaving && !leaving.spectator) {
      pushSystemMsg(room, `${leaving.name} uzildi, qaytishi kutilmoqda...`);
      broadcastRoom(code);
      const timeoutHandle = setTimeout(() => {
        delete pendingRejoin[nameLower];
        const staleSocket = io.sockets.sockets.get(socket.id) || socket;
        removePlayerFromRoom(staleSocket);
      }, RECONNECT_GRACE_MS);
      pendingRejoin[nameLower] = { code, oldSocketId: socket.id, timeoutHandle };
      return;
    }
  }
...
// Rebinds a returning account's old, now-stale player entry (in both
// room.players and, if a match is live, room.phys.players) onto their new
// socket.id. Returns a small payload the client can use to resume exactly
// where it left off, or null if there was nothing to resume.
function tryResumePendingRoom(socket, nameLower) {
  const pending = pendingRejoin[nameLower];
  if (!pending) return null;
  const room = rooms[pending.code];
  if (!room) { clearTimeout(pending.timeoutHandle); delete pendingRejoin[nameLower]; return null; }
  clearTimeout(pending.timeoutHandle);
  delete pendingRejoin[nameLower];
  const p = room.players.find(pp => pp.id === pending.oldSocketId);
  if (!p) return null;
  p.id = socket.id;
  socketRoom[socket.id] = pending.code;
  socket.join(pending.code);
  if (room.hostId === pending.oldSocketId) room.hostId = socket.id;
  if (room.phys) {
    const phy = room.phys.players.find(pp => pp.netId === pending.oldSocketId);
    if (phy) phy.netId = socket.id;
  }
  delete roomInputs[pending.oldSocketId];
  pushSystemMsg(room, `${p.name} qaytib keldi`);
  broadcastRoom(pending.code);
  return { code: pending.code, room: publicRoomState(room), myId: socket.id, isHost: room.hostId === socket.id, spectator: !!p.spectator };
}
...
io.on('connection', (socket) => {
  ...
  socket.on('login', async ({ name, pass, seed }, cb) => {
    try {
      ...
      const rejoined = tryResumePendingRoom(socket, nameLower);
      const { passHash, ...safe } = account;
      cb({ ok: true, account: safe, rejoined });
